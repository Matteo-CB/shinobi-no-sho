...
### Integrite + couverture

- **0 reference cassee** entre datasets (verifiable via `python scripts/repair_canon_refs.py report-only`)
- Chaque entite a ses **wiki_sections** (Background, Personality, Appearance, Abilities, Trivia, Part I/II, etc.) = ~7.3 Mo de contenu Narutopedia exploitable par le RAG
- 112/1360 NPCs majeurs ont leur **birth_year** canon (les autres sont mineurs, manquants dans le scraping initial)
- 341/1360 ont leurs **natures de chakra** + 184/1360 leurs **kekkei genkai** detectes via wiki_links
- **15 940 chunks RAG** indexes (1 par section wiki par entite + chunks principaux)

### Maintenance

```powershell
# Audit referentiel canon
python scripts/repair_canon_refs.py report-only

# Reparer les ids inverses (sakura_haruno -> haruno_sakura)
python scripts/repair_canon_refs.py repair

# Purger les refs vers des NPCs inexistants
python scripts/purge_broken_refs.py

# Rebuild canon depuis les wikitext scrapes (data/raw/narutopedia/)
python scripts/build_canonical_jsons.py

# Build + archive l'index RAG pour publication GitHub Releases
python scripts/build_rag_index.py archive-only
```
