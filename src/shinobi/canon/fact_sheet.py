...
def fact_sheet_for(canon: CanonBundle, character_id: str, *, current_year: int) -> str | None:
    ...
    if char.birth_year is not None and current_year < char.birth_year:
        lines.append(f"  STATUT: PAS ENCORE NE (naissance prevue an {char.birth_year}). Ne peut PAS apparaitre dans la scene.")
        return "\n".join(lines)
    ...
    # Notes psychologiques canoniques (le plus important pour eviter les inventions)
